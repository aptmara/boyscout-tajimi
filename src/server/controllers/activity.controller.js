...
const deleteActivity = asyncHandler(async (req, res) => {
  // 削除前に画像URLを取得
  const { rows } = await db.query(
    `SELECT image_urls FROM activities WHERE id = $1`,
    [req.params.id]
  );
  ...
  if (rows.length > 0 && rows[0].image_urls) {
    ...
    deleteImages(imageUrls).catch(err =>
      console.error('[DeleteActivity] Image cleanup failed:', err)
    );
  }
...
const activityWebhook = asyncHandler(async (req, res) => {
  ...
  await db.query(
    ...
    [String(title), processedContent, JSON.stringify(imgs), cat, uni || null, JSON.stringify(tgs), ad]
  );
