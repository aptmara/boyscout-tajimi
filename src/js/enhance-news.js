...
async function loadDynamicNewsList(){
  ...
  const renderPagination = (total, page) => {
    ...
    newsPaginationContainer.querySelectorAll('.news-pagination-button').forEach(btn=>{
      btn.addEventListener('click',(e)=>{
        ...
        const totalPages = Math.ceil(allNews.length / NEWS_ITEMS_PER_PAGE);
        if (page && page!==currentPage && page>0 && page<=totalPages){ currentPage=page; displayNewsItems(allNews); container.scrollIntoView({behavior:'smooth'}); }
      });
    });
  };
  ...
  const renderSkeletons = (n=6) => {
    const cards=[]; for(let i=0;i<n;i++){ cards.push(`<article class="bg-white rounded-xl shadow-lg overflow-hidden"><div class="skeleton skeleton-thumb"></div><div class="p-6"><div class="skeleton skeleton-text" style="width:35%"></div><div class="skeleton skeleton-text" style="width:90%"></div><div class="skeleton skeleton-text" style="width:80%"></div></div></article>`);} container.innerHTML = cards.join('');
  };
  ...
  try {
    ...
    const q = readQuery();
    // UIへ反映
    const catSel = document.getElementById('news-category-filter'); const unitSel = document.getElementById('news-unit-filter'); const tagSel = document.getElementById('news-tags-filter'); const monthInp = document.getElementById('news-date-filter'); const searchInp = document.getElementById('news-search');
    if (catSel && q.category) catSel.value = q.category; if (unitSel && q.unit) unitSel.value = q.unit; if (Array.isArray(q.tags_any) && tagSel) Array.from(tagSel.options).forEach(o=>o.selected = q.tags_any.includes(o.value)); if (q.ym && monthInp) monthInp.value = q.ym; if (q.q && searchInp) searchInp.value = q.q;
    displayNewsItems(allNews);
  } catch (err) { console.error('Failed to fetch news:', err); container.innerHTML = '<p class="text-center text-red-500">お知らせの読み込みに失敗しました。</p>'; }

  // フィルタ
  const btn = document.getElementById('news-filter-button'); const catSel = document.getElementById('news-category-filter'); const monthInp = document.getElementById('news-date-filter'); const unitSel = document.getElementById('news-unit-filter'); const tagSel = document.getElementById('news-tags-filter'); const searchInp = document.getElementById('news-search'); const btnText = document.getElementById('filter-button-text'); const spinner = document.getElementById('filter-loading-spinner');
  if (btn) btn.addEventListener('click', ()=>{
    ...
    } finally { spinner?.classList?.add('hidden'); if (btnText) btnText.textContent = '絞り込む'; }
  });
}
