...
(function () {
  ...
  const Palette = Admin.palette = {
    ...
    execute(item) { if (!item) return; this.close(); if (item.type === 'view') { Admin.views?.setActiveView(item.key, { force: true }); return; } if (item.type === 'news') { Admin.editors?.openNews(item.key); return; } if (item.type === 'activity') { Admin.editors?.openActivity(item.key); } },
