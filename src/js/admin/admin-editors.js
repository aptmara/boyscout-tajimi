...
(function(){
  ...
  async function openNewsEditor(id){
    ...
    const html = `
      <form id="${formId}" class="editor-form">
        <div class="form-row"><label>タイトル</label><input type="text" id="news-title" required class="input"></div>
        <div class="form-row"><label>本文</label><textarea id="news-content" required class="textarea" rows="8"></textarea></div>
        <div class="form-row"><label>カテゴリ</label><input type="text" id="news-category" class="input" placeholder="未分類"></div>
        <div class="form-row"><label>隊</label><select id="news-unit" class="input"><option value="">（未指定）</option></select></div>
        <div class="form-row"><label>タグ（複数選択可）</label><select id="news-tags" class="input" multiple size="5"></select></div>
        <div class="editor-actions"><button type="submit" class="btn">${isEdit?'更新する':'作成する'}</button><button type="button" id="news-cancel" class="btn-ghost">キャンセル</button></div>
    ...
    onSheetReady(async ()=>{
      ...
      try{
        ...
        unitSelect.innerHTML = '<option value="">（未指定）</option>' + (Array.isArray(units)?units.map(u=>`<option value="${utils.escapeHtml(u.slug)}">${utils.escapeHtml(u.label||u.slug)}</option>`).join(''): '');
  ...
  async function openActivityEditor(id){
    const isEdit = Boolean(id);
    const title = isEdit ? '活動を編集' : '活動を作成';
    const formId = 'activity-editor-form';
    const html = `
      <form id="${formId}" class="editor-form">
        <div class="form-row"><label>タイトル</label><input type="text" id="act-title" required class="input"></div>
        <div class="form-row"><label>本文</label><textarea id="act-content" required class="textarea" rows="8"></textarea></div>
        <div class="form-row"><label>カテゴリ</label><input type="text" id="act-category" class="input" placeholder="未分類"></div>
        <div class="form-row"><label>隊</label><select id="act-unit" class="input"><option value="">（未指定）</option></select></div>
        <div class="form-row"><label>タグ（複数選択可）</label><select id="act-tags" class="input" multiple size="5"></select></div>
        <div class="form-row"><label>実施日</label><input type="date" id="act-date" class="input"></div>
        <div class="editor-actions"><button type="submit" class="btn">${isEdit?'更新する':'作成する'}</button><button type="button" id="act-cancel" class="btn-ghost">キャンセル</button></div>
    ...
    onSheetReady(async ()=>{
      ...
      if (isEdit){
        try {
          ...
          } else { err.textContent = '活動の読み込みに失敗しました。'; }
        } catch { err.textContent = '活動の読み込みに失敗しました。'; }
      }
