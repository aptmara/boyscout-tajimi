...
test('resolveSessionSecret requires explicit production secret', () => {
  assert.equal(resolveSessionSecret({
    NODE_ENV: 'development',
    SESSION_SECRET: ' dev-secret ',
  }), 'dev-secret');

  assert.throws(() => resolveSessionSecret({ NODE_ENV: 'production' }), /SESSION_SECRET must be set/);
  assert.match(resolveSessionSecret({ NODE_ENV: 'development' }), /^[a-f0-9]{64}$/);
});
...
test('sanitizePayload strips active content while preserving safe placeholders', () => {
  const payload = sanitizePayload({
    ...
    content: '<p onclick="evil()">Hi<script>alert(1)</script><img src="https://example.com/a.png" data-image-index="0" onerror="x"></p>',
  });
